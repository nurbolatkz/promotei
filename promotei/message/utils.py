...
def create_or_update_message(sender, receiver, contract):
    ...
    if contract.status == 'CREATED':
        ...
        message_content = f'{sender.indentity_number.first_name} {sender.indentity_number.last_name}  принял(-а) договор'
    elif contract.status == 'DECLINED':
        is_accepted_or_declined =  True
        message_content = f'{sender.indentity_number.first_name} {sender.indentity_number.last_name} +  отклонил(-а) договор'
